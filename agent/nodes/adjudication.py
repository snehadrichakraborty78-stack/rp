...
def _build_system_prompt(
    ...
    *,
    ...
    cand_lines = []
    for c in candidates:
        ctype = c.get('entity_type', '?') if isinstance(c, dict) else getattr(c, 'entity_type', '?')
        cid = c.get('entity_id', '?') if isinstance(c, dict) else getattr(c, 'entity_id', '?')
        cpaise = c.get('amount_paise', 0) if isinstance(c, dict) else getattr(c, 'amount_paise', 0)
        cscore = c.get('score', 0.0) if isinstance(c, dict) else getattr(c, 'score', 0.0)
        cand_lines.append(
            f"  - {ctype}:{cid} "
            f"amount={cpaise} paise  "
            f"score={cscore:.3f}"
        )
